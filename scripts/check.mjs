...
const required=['README.md','AGENTS.md','REPOSITORY.yaml','LICENSE','SECURITY.md','CONTRIBUTING.md','wrangler.jsonc','src/worker.js','public/index.html','public/app.js','public/styles.css','public/claryel-standard.css','public/claryel-standard.js','public/assets/claryel-mark-v3.svg','public/i18n/manifest.json','docs/ARCHITECTURE.md','docs/AI_APP_WORKFLOW.md','docs/DEPLOYMENT.md','docs/LOCALIZATION.md','docs/MARKET_POSITIONING.md','docs/PRIVATE_EXPORT_BOUNDARY.md','docs/CLARYEL_UNIVERSE.md','docs/BOX_BASELINE.md','NEXT_STEPS.md'];
for(const file of required)await access(path.join(root,file));
const read=file=>readFile(path.join(root,file),'utf8');
const worker=await read('src/worker.js');const wrangler=await read('wrangler.jsonc');const html=await read('public/index.html');const standardCss=await read('public/claryel-standard.css');const standardJs=await read('public/claryel-standard.js');const app=await read('public/app.js');const license=await read('LICENSE');const architecture=await read('docs/ARCHITECTURE.md');const universeDocs=await read('docs/CLARYEL_UNIVERSE.md');const baselineDocs=await read('docs/BOX_BASELINE.md');const deployment=await read('docs/DEPLOYMENT.md');const aiWorkflow=await read('docs/AI_APP_WORKFLOW.md');
...
const manifest=JSON.parse(await read('public/i18n/manifest.json'));const expected=['en','it','de','fr','es','nl','pt','pl','ro','cs','sv','el','da','fi','zh-CN','hi','ar','id','uk','ru'];
if(manifest.surface!=='classic')throw new Error('Community application manifest must declare the classic page surface.');
...
for(const item of manifest.public){
  ...
  if(item.path!==expectedPath)throw new Error(`Non-standard classic locale path for ${item.code}: ${item.path}`);
  await access(path.join(root,`public/assets/flags/${item.flag}.svg`));
}
const english=JSON.parse(await read('public/i18n/en.json'));const keys=Object.keys(english).sort();
for(const code of expected){const catalogue=JSON.parse(await read(`public/i18n/${code}.json`));const missing=keys.filter(key=>typeof catalogue[key]!=='string'||!catalogue[key].trim());if(missing.length)throw new Error(`Locale ${code} is missing: ${missing.join(', ')}`);}
...
for(const marker of['archive/community-before-box-clone-20260801','https://claryel.com','/classic/','same-origin proxy','independently buildable'])if(!baselineDocs.includes(marker))throw new Error(`Box baseline documentation is missing ${marker}`);
if(!deployment.includes('BOX_ORIGIN')||!deployment.includes('/classic/'))throw new Error('Deployment documentation does not cover the Box proxy and preserved route.');
