...
const SEO=Object.freeze({
  ...
  hi:{title:"CLARYEL Web Community — आवाज़ से वेबसाइट बनाएँ और प्रबंधित करें",description:"अपनी वेबसाइट का वर्णन करें, GitHub और Cloudflare से प्रकाशित करें और आवाज़ से सामग्री, डिज़ाइन तथा कार्यक्षमता बदलें।"},
  ar:{title:"CLARYEL Web Community — أنشئ المواقع وأدرها بالصوت",description:"صِف الموقع المطلوب وانشره عبر GitHub وCloudflare ثم غيّر المحتوى والتصميم والوظائف بالصوت."},
  id:{title:"CLARYEL Web Community — Buat dan kelola situs dengan suara",description:"Jelaskan situs yang Anda inginkan, terbitkan melalui GitHub dan Cloudflare, lalu ubah konten, desain, dan fungsi dengan suara."},
  uk:{title:"CLARYEL Web Community — створюйте сайти й керуйте ними голосом",description:"Опишіть потрібний сайт, опублікуйте його через GitHub і Cloudflare та змінюйте вміст, дизайн і функції голосом."},
...
function withSecurityHeaders(response,options={}){const headers=new Headers(response.headers);for(const [name,value] of Object.entries(SECURITY_HEADERS))headers.set(name,value);headers.set("Cache-Control",options.cacheControl||"public, max-age=60, must-revalidate");if(options.contentLanguage)headers.set("Content-Language",options.contentLanguage);if(options.noIndex)headers.set("X-Robots-Tag","noindex, nofollow, noarchive");return new Response(response.body,{status:response.status,statusText:response.statusText,headers})}
...
function redirectForLegacyLanguage(url){if(!url.searchParams.has("lang"))return null;const code=normaliseCode(url.searchParams.get("lang"));const target=LOCALES[code];if(!target)return null;const destination=new URL(target.path,url.origin);for(const [key,value] of url.searchParams){if(key!=="lang")destination.searchParams.append(key,value)}return Response.redirect(destination.toString(),308)}
function canonicalSlashRedirect(url){const code=localeFromPath(url.pathname);const meta=LOCALES[code];if(code!=="en"&&url.pathname.toLowerCase()===meta.path.slice(0,-1)){const destination=new URL(meta.path,url.origin);destination.search=url.search;return Response.redirect(destination.toString(),308)}return null}
function createRobots(origin){return new Response(`User-agent: *\nAllow: /\nSitemap: ${origin}/sitemap.xml\n`,{headers:{"Content-Type":"text/plain; charset=utf-8"}})}
function createSitemap(origin){const entries=PUBLIC_LOCALES.map(code=>`  <url><loc>${origin}${LOCALES[code].path}</loc></url>`).join("\n");return new Response(`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${entries}\n</urlset>\n`,{headers:{"Content-Type":"application/xml; charset=utf-8"}})}
function hreflangMarkup(origin){const links=PUBLIC_LOCALES.map(code=>`  <link rel="alternate" hreflang="${LOCALES[code].locale}" href="${origin}${LOCALES[code].path}">`);links.push(`  <link rel="alternate" hreflang="x-default" href="${origin}/">`);return links.join("\n")}
function replaceMeta(html,{code,origin}){const meta=LOCALES[code];const seo=SEO[code]||SEO.en;const canonical=`${origin}${meta.path}`;return html.replace(/<html\b[^>]*>/i,`<html lang="${meta.locale}"${meta.direction==='rtl'?' dir="rtl"':''} data-site="community" data-locale="${code}" data-hidden-locale="false">`).replace(/<title>[^<]*<\/title>/,`<title>${escapeHtml(seo.title)}</title>`).replace(/(<meta name="description" content=")[^"]*(">)/,`$1${escapeAttribute(seo.description)}$2`).replace(/(<meta name="robots" content=")[^"]*(">)/,`$1index,follow,max-image-preview:large$2`).replace(/(<meta property="og:title" content=")[^"]*(">)/,`$1${escapeAttribute(seo.title)}$2`).replace(/(<meta property="og:description" content=")[^"]*(">)/,`$1${escapeAttribute(seo.description)}$2`).replace(/(<meta property="og:url" content=")[^"]*(">)/,`$1${canonical}$2`).replace(/(<meta property="og:locale" content=")[^"]*(">)/,`$1${meta.ogLocale}$2`).replace(/(<link rel="canonical" href=")[^"]*(">)/,`$1${canonical}$2`).replace("  <meta name=\"claryel-hreflang-placeholder\" content=\"\">",hreflangMarkup(origin))}
function escapeHtml(value=""){return String(value).replace(/[&<>]/g,character=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[character]))}
function escapeAttribute(value=""){return escapeHtml(value).replace(/"/g,"&quot;")}

export async function handleRequest(request,env){
  const url=new URL(request.url);const origin=env.PUBLIC_ORIGIN||url.origin;if(request.method!=="GET"&&request.method!=="HEAD")return withSecurityHeaders(json({error:"method_not_allowed"},405),{cacheControl:"no-store"});
  const legacyRedirect=redirectForLegacyLanguage(url);if(legacyRedirect)return withSecurityHeaders(legacyRedirect,{cacheControl:"no-store"});const slashRedirect=canonicalSlashRedirect(url);if(slashRedirect)return withSecurityHeaders(slashRedirect,{cacheControl:"no-store"});
  if(url.pathname==="/api/health")return withSecurityHeaders(json({status:"ok",product:"CLARYEL Web Community",version:env.PRODUCT_VERSION||"0.3.0"}),{cacheControl:"no-store"});
  if(url.pathname==="/api/public-config")return withSecurityHeaders(json({product:"CLARYEL Web Community",edition:"community",freeSiteLimit:Number.parseInt(env.FREE_SITE_LIMIT||"2",10),freeLimitBasis:"account-holder",publicOrigin:origin,localePaths:Object.fromEntries(PUBLIC_LOCALES.map(code=>[code,LOCALES[code].path])),publicLocales:PUBLIC_LOCALES,hiddenLocales:[],universeUrl:"https://claryel.space/universe/",aiMode:"voice-first-chatgpt-application"}),{cacheControl:"public, max-age=60, must-revalidate"});
  if(url.pathname==="/robots.txt")return withSecurityHeaders(createRobots(origin),{cacheControl:"public, max-age=3600"});if(url.pathname==="/sitemap.xml")return withSecurityHeaders(createSitemap(origin),{cacheControl:"public, max-age=3600"});
  const code=localeFromPath(url.pathname);let assetResponse=await env.ASSETS.fetch(request);const isDocument=!url.pathname.includes(".");if(assetResponse.status===404&&isDocument)assetResponse=await env.ASSETS.fetch(new Request(new URL("/index.html",url),request));
  if(isDocument&&assetResponse.ok){const html=replaceMeta(await assetResponse.text(),{code,origin});assetResponse=new Response(request.method==="HEAD"?null:html,{status:assetResponse.status,headers:assetResponse.headers})}
